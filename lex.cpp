...
int main()
{
	...
	while(!ifile.eof()&&state!=3)
	{
		...
		pi.first=re;
		pi.second=action;
		reaction.insert(pi);//规则部分把模式和动作分别存储在reaction中，first存re，second存action
	...
	cout<<"NFA构建完成"<<endl;
	cout<<"开始由NFA构建DFA"<<endl;
	DFA dfa;
    nfa1.nfaTodfa(dfa);
	dfa.subsetCon();
	cout<<"DFA构建完成"<<endl;
	cout<<"开始DFA最小化操作"<<endl;
    dfa.minimizeDfa();
	cout<<"DFA最小化操作完成"<<endl;
	cout<<"开始写入cpp程序"<<endl;
	CreateCode(dfa);
	ifile.get();
	while(!ifile.eof())
...
void CreateCode(DFA& dfa)//to generate code according to DFA
{
	...
	for (int unsigned i=0;i<dfa.dfaNodeTable.size();i++)   //对每一个dfa状态都有一个case
	{
		int flagofelseif=0;
		...
		for(unsigned int k=0;k<curNode.edges.size();k++)//逐个扫描该结点的边
		{
