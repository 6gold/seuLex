...
void DFA::minimizeDfa()
{
	...
	int size = this -> dfaNodeTable.size();								//DFA的结点表
	int startId = dfaNodeTable[0].label;								//初始标号值
	...
	for(unsigned int j=0;j<stateToSet.size();j++)//输出接点号与结点集号
	{
	...
	*/
	...
	/*	for(unsigned int j=0;j<stateToSet.size();j++)//下面根据分划表生成dfa结点表
	{
	cout<<"结点"<<j<<"  集合 "<<stateToSet[j]<<endl;
	}
	*/
...
//////////////此处效率太低，需要改进
/***判断结点集U中是否含有终止结点***/
bool DFA::isTerminate(set<int> U, vector<int> &terSet, int &terNum)
{
	terNum = 0;
	terSet.clear();
...
/***求一个状态集合的epslon闭包***/
set<int> DFA::epslonClosure(set<int> T)
{
	set<int> closure;
...
/***求一个结点的epslon闭包***/
set<int> DFA::epslonClosure(int s)
{
	set<int> closure;
	closure.insert(s);
	int edgeNum;//=dfaNodeTable[s].edges.size();
	...
	while (!nodeQue.empty())//当未搜索的结点队列不为空时
	{
		...
		for (int i = 0; i<edgeNum; i++)//搜索当前结点的所有边
		{
			tempEdge = dfaNodeTable[nodeNum].edges[i];
			int next = tempEdge.nextNode;//下一个结点标号
			if (tempEdge.symbol == '@' && !isIn(closure, next))//若当前边上lable是epslon，且下一个结点标号不在closure中
			{
				closure.insert(next);
				nodeQue.push_back(next);
			}
